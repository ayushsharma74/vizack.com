...
export default function Navbar() {
    ...
    const [menu, setMenu] = useState("icofont-navigation-menu");
    let z = 0;
    
    const clickCall = (e) => {
        let list = e.target.parentNode.nextElementSibling.children;
        ...
        if (window.innerWidth < 1024 && z === 0 && e.target.classList.contains("icofont-navigation-menu")) {
            setTimeout(() => {
                ...
                e.target.parentNode.nextElementSibling.classList.remove("max-lg:hidden", "aos-animate")
                setTimeout(() => {
                    e.target.parentNode.nextElementSibling.classList.add("max-lg:h-full",
                    "max-lg:w-full", "max-lg:flex", "max-lg:flex-col",
                    "max-lg:align-middle", "max-lg:justify-start", "max-lg:gap-20",
                    "max-lg:my-24", "max-lg:absolute", "aos-animate")
                    li.forEach(element => {
                        element.children[0].classList.remove("aos-animate")
                        setTimeout(() => {
                            element.children[0].classList.add("aos-animate")
                        },2)
                        e.target.parentNode.nextElementSibling.children[0].children[0].classList.add("mt-4")
                    });
                },2)
            ...
            setTimeout(() => {
                ...
                e.target.parentElement.classList.add("aos-animate")
                e.target.parentNode.nextElementSibling.classList.add("max-lg:hidden", "aos-animate")
                e.target.parentNode.nextElementSibling.classList.remove("max-lg:h-full",
                    "max-lg:w-full", "max-lg:flex", "max-lg:flex-col",
                    "max-lg:align-middle", "max-lg:justify-start", "max-lg:gap-20",
                    "max-lg:my-24", "max-lg:absolute", "aos-animate")
                    li.forEach(element => {
                    ...
                    e.target.parentNode.nextElementSibling.children[0].children[0].classList.remove("mt-4")
    ...
    return (
        <nav className="w-full h-24 flex justify-evenly max-lg:justify-between">
