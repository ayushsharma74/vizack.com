    ...
    return (
        ...
        <Link href={props.route}><Image src={props.imgSrc} alt={props.title} width={250} height={250} loader={imgloader} loading="lazy" className="md:hidden lg:hidden w-full h-full" /></Link>
            <div className="flex items-start flex-col gap-4">
                ...
                <Link href={`/blogs/${props.route}`}><h1 onMouseOver={toggleArrow} onMouseOut={toggleArrow} className="blog-title font-bold relative">{props.title} <span className="font-extrabold text-4xl absolute -top-3 transition hidden animate-arrow text-default">&rarr;</span></h1></Link>
                <Link href={`/blogs/${props.route}`}><p className="text-sm">{props.disc}</p></Link>
                <Link href={`/blogs/${props.route}`} className="underline decoration-2 hover:decoration-orange-500 hover:text-black transition-colors duration-150">Read More</Link>
            </div>
        </div>
